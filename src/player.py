...
class PlayerManager:
    def __init__(self, socketio) -> None:
        ...
        self.db = TinyDB('instance/players.json')
    ...
    def findExistingPlayer(self, playerName):
        User = Query()
        results = self.db.search(User.username.matches(playerName, flags=re.IGNORECASE))

        if len(results) == 1:
            return Player(**results[0])
        return None
            # return self.db.search(User.username.upper() == playerName.upper())
